...
function getUsersInRoom(room) {
    ...
    if (clients) {
        clients.forEach((socketId) => {
            ...
            const info =  {name : userSocket.user, id : socketId };
            users.push(info);
        });
    }
    return users;
...
io.on('connection', (socket) => {
    ...
    socket.on('sendMessage', (message) => {
        ...
        if (message.user === 'all') {
            io.to(socket.room).emit('newMessage', message.message, message.nick, message.user, false);
        } else {
            io.to(message.user).emit('newMessage', message.message, message.nick, message.user, true);
            //자기자신한테 메세지 띄우기
            socket.emit('newMessage', message.message, message.nick, message.user, true);
        }
    });
